...
pub unsafe extern "C" fn kernel_trap() {
    let sepc_val = Csr::Sepc.read();
    let scause_val = Csr::Scause.read();
    let stval_val = Csr::Stval.read();
    let sstatus_val = Csr::Sstatus.read();

    if sstatus_val & (1 << 8) == 0 {
        println!("sepc:     {:#018x}", sepc_val);
        println!("scause:   {:#018x}", scause_val);
        println!("stval:    {:#018x}", stval_val);
